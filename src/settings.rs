...
pub fn load_from_flash<T: for<'d> miniconf::JsonCoreSlash<'d, Y>, const Y: usize>(
    structure: &mut T,
    storage: &mut Flash,
) {
    ...
    for path in T::iter_paths::<heapless::String<64>>("/") {
        ...
        let mut deserializer =
            postcard::Deserializer::from_flavor(postcard::de_flavors::Slice::new(&item.data));
        if let Err(e) = structure.deserialize_by_key(path.split('/').skip(1), &mut deserializer) {
...
impl<C, const Y: usize> serial_settings::Platform<Y> for SerialSettingsPlatform<C, Y>
where
    C: serial_settings::Settings<Y>,
{
    type Interface = serial_settings::BestEffortInterface<crate::hardware::SerialPort>;
    type Settings = C;
    type Error = Error<<LockedFlashBank as embedded_storage::nor_flash::ErrorType>::Error>;
    ...
    fn save(&mut self, buf: &mut [u8], settings: &Self::Settings) -> Result<(), Self::Error> {
        for path in Self::Settings::iter_paths::<heapless::String<64>>("/") {
            ...
            if let Err(e) = settings.serialize_by_key(item.path.split('/').skip(1), &mut serializer)
            {
            ...
            .unwrap_or(true)
            {
                ...
                sequential_storage::map::store_item(&mut self.storage, range, buf, item).unwrap();
            }
        }
