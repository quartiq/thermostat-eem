...
use idsp::{
    AccuOsc, Sweep,
    iir::{
        Biquad, BiquadClamp, DirectForm1,
        pid::{Pid, Units},
        repr::BiquadRepr,
    },
};
...
pub struct OutputChannel {
    ...
    pub biquad: BiquadRepr<f32, f64, f64>,

    /// Built biquad
    #[tree(skip)]
    pub iir: BiquadClamp<f64>,

    #[tree(skip)]
    iir_state: DirectForm1<f64>,
...
impl Iterator for SineSweep {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.sweep.next().map(|c| c.0[1] as f32 * SCALE * self.amp)
    }
}
...
impl OutputChannel {
    ...
    pub fn update(&mut self, temperatures: &[[f64; 4]; 4]) -> f32 {
        let temperature = temperatures
            ...
            .map(|(t, w)| t * *w as f64)
            .sum();
        let y = if self.state == State::On {
            self.iir.process(&mut self.iir_state, temperature)
        } else {
            Biquad::<f64>::HOLD.process(&mut self.iir_state, temperature)
        };
    ...
    pub fn error(&self) -> f32 {
        (self.iir_state.xy[0] as f32 + self.iir_state.xy[1] as f32) * 0.5
            + self.iir.input_offset() as f32
    }
}

mod validate_biquad {
    use crate::output_channel::UNITS;

    use super::{BiquadRepr, DacCode, MAX_CURRENT_LIMIT, OutputChannel};
    use miniconf::{
    ...
    pub const SCHEMA: &'static Schema = BiquadRepr::<f32, f64>::SCHEMA;
    ...
    pub fn deserialize_by_key<'de, D: Deserializer<'de>>(
        ...
        value.iir = value.biquad.build(&UNITS);
        let range = DacCode::MAX_CURRENT.min(MAX_CURRENT_LIMIT);
        value.iir.max = value.iir.max.clamp(-range as _, range as _);
        value.iir.min = value.iir.min.clamp(-range as _, range as _);
        Ok(())
    }
    ...
    pub fn probe_by_key<'de, T: ?Sized, D: Deserializer<'de>>(
        ...
        BiquadRepr::<f32, f64>::probe_by_key(keys, de)
    }
}
